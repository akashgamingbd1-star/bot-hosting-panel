...
def init_db():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS bots (
            ...
            log_output TEXT DEFAULT 'No logs yet.',
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', sans-serif; }
        ...
        .console-box { background: #050811; border: 1px solid #1f2937; border-radius: 8px; padding: 12px; font-family: monospace; font-size: 12px; color: #10b981; max-height: 150px; overflow-y: auto; white-space: pre-wrap; margin-top: 15px; }
    </style>
</head>
<body>
...
<div class="container">
    ...
    <div class="grid">
        <div class="card">
            <div class="card-header">📦 Upload Both Bot Files</div>
            <form action="/upload" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    <label>Bot Display Name</label>
                    <input type="text" name="bot_name" placeholder="e.g. OTP Master Bot" required>
                </div>
                <div class="form-group">
                    <label>Select main.py AND requirements.txt</label>
                    <input type="file" name="bot_files" multiple required>
                </div>
                <button type="submit" class="btn">Deploy & Install Packages</button>
            </form>
        </div>
        ...
        <div class="card">
            ...
            <div style="margin-top: 20px;">
                <label style="font-size: 12px; color: var(--text-muted); font-weight: 600;">System Console Logs:</label>
                <div class="console-box">{% if bots %}{{ bots[0]['log_output'] }}{% else %}Ready for deployment...{% endif %}</div>
            </div>
            {% else %}
            <div style="text-align: center; padding: 40px; color: var(--text-muted);">
                No bots deployed yet. Select both files and click Deploy!
            </div>
            {% endif %}
        </div>
    </div>
...
def upload():
    ...
    files = request.files.getlist('bot_files')
    ...
    for file in files:
        if file.filename:
            ...
            # Auto-rename requirements files containing numbers like (1)
            if 'requirements' in fname.lower():
            ...
            if fname == 'requirements.txt':
                try:
                    log_msg += "Installing packages from requirements.txt...\\n"
                    result = subprocess.run([sys.executable, "-m", "pip", "install", "-r", save_path], capture_output=True, text=True)
                    log_msg += "Packages Installed Successfully!\\n"
                except Exception as e:
                    log_msg += f"Package install error: {str(e)}\\n"

    if main_filename:
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute('INSERT INTO bots (bot_name, filename, status, log_output) VALUES (?, ?, ?, ?)',
                       (bot_name, main_filename, 'Stopped', log_msg))
        conn.commit()
        conn.close()
...
def start_bot(bot_id):
    ...
    if bot:
        ...
        if bot_id in active_processes and active_processes[bot_id].poll() is None:
            ...
            cursor.execute('UPDATE bots SET status = "Running", log_output = "Bot process started successfully!" WHERE id = ?', (bot_id,))
            conn.commit()
...
def stop_bot(bot_id):
    ...
    conn = get_db()
    cursor = conn.cursor()
    cursor.execute('UPDATE bots SET status = "Stopped", log_output = "Bot process stopped." WHERE id = ?', (bot_id,))
    conn.commit()
    conn.close()
