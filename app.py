    ...
    <div class="grid">
        <div class="card">
            ...
            <form action="/upload" method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label>Bot Files (Select main.py & requirements.txt together)</label>
                    <input type="file" name="bot_files" multiple required>
                </div>
                <button type="submit" class="btn">Upload & Deploy All</button>
            </form>
        </div>
        ...
        <div class="card">
                ...
                <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            <div style="font-size: 11px; color: var(--text-muted);">Main Script: {{ bot['filename'] }}</div>
                        </td>
                        <td>
            ...
            <div style="text-align: center; padding: 40px; color: var(--text-muted);">
                No bots deployed yet. Select your bot files from the left panel to get started!
            </div>
            {% endif %}
        </div>
    </div>
...
def upload():
    ...
    for file in files:
        if file.filename:
            # If uploaded requirement file contains (1) or spaces, clean filename
            fname = file.filename
            ...
            # Auto install requirements if requirements.txt is uploaded
            if fname == 'requirements.txt':
                try:
                    subprocess.run([sys.executable, "-m", "pip", "install", "-r", save_path], check=True)
    ...
    if main_filename:
        ...
        cursor.execute('INSERT INTO bots (bot_name, filename, status) VALUES (?, ?, ?)',
                       (bot_name, main_filename, 'Stopped'))
        conn.commit()
        conn.close()
