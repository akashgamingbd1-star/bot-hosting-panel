...
def upload():
    ...
    if req_file and req_file.filename:
        ...
        try:
            ...
            subprocess.run([sys.executable, "-m", "pip", "install", "--no-cache-dir", "-r", req_path], capture_output=True, text=True)
...
def start_bot(bot_id):
    ...
    if bot:
        ...
        if bot_id not in active_processes or active_processes[bot_id]['process'].poll() is not None:
            # High-performance environment configuration for heavy python execution
            env = os.environ.copy()
            ...
            cursor.execute('UPDATE bots SET status = "Running", logs = "Bot is running live (Optimized Runtime)!", start_timestamp = ? WHERE id = ?', (current_time, bot_id))
            conn.commit()
